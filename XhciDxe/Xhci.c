...
//
// Template for Xhci's Usb Host Controller Protocol Instance.
//
EFI_USB_HC_PROTOCOL gXhciUsbHcTemplate = {
  hcReset,
  hcGetState,
  hcSetState,
  hcControlTransfer,
  hcBulkTransfer,
  hcAsyncInterruptTransfer,
  hcSyncInterruptTransfer,
  hcIsochronousTransfer,
  hcAsyncIsochronousTransfer,
  hcGetRootHubPortNumber,
  hcGetRootHubPortStatus,
  hcSetRootHubPortFeature,
  hcClearRootHubPortFeature,
  0x3,
  0x0
};
  ...
  )
{
  ...
  case EfiUsbHcStateOperational:
    ...
    //
    if (!XHC_REG_BIT_IS_SET (Xhc, XHC_USBSTS_OFFSET, XHC_USBSTS_HALT)) {
      DEBUG((EFI_D_INFO, "bit halt is not set\n"));
      Status = EFI_DEVICE_ERROR;
      break;
    }
  ...
  )
{
  ...
  default:
    //DEBUG((EFI_D_INFO, "XhcGetRootHubPortStatus: XHC_PORTSC_PS:%d\n", State & XHC_PORTSC_PS));
    break;
  }
...
ON_EXIT:
  DEBUG ((EFI_D_INFO, "XhcClearRootHubPortFeature for port %d Feature=%x: status %r\n", PortNumber, PortFeature, Status));
  gBS->RestoreTPL (OldTpl);
  ...
  )
{
  ...
  *TransferResult = EFI_USB_ERR_SYSTEM;
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XHC transfer failed\n"));
    goto ON_EXIT;
  }
  ...
  )
{
  ...
  CopyMem (&Xhc->UsbHc, &gXhciUsbHcTemplate, sizeof (EFI_USB_HC_PROTOCOL));
  ...
  if (!EFI_ERROR (Status)) {
    ...
    Xhc->UsbHc.MajorRevision = (ReleaseNumber & 0xF0) >> 4;
    Xhc->UsbHc.MinorRevision = (ReleaseNumber & 0x0F);
  }
  ...
  DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: Xhc HCI version=%x\n", XhcReadCapReg8(Xhc, XHC_HCIVERSION_OFFSET)));
  ...
  DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: Max Ports %d\n", Xhc->HcSParams1.Data.MaxPorts));
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((EFI_D_ERROR, "XhcDriverBindingStart: failed to install USB_HC and USB2_HC Protocols\n"));
    goto FREE_POOL;
  }
...
{
  DEBUG((DEBUG_INFO, "hcReset\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcReset (&Xhc->Usb2Hc, Attributes);
  return Status;
}
...
{
  DEBUG((DEBUG_INFO, "hcGetState\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcGetState (&Xhc->Usb2Hc, State);
  return Status;
}
...
{
  DEBUG((DEBUG_INFO, "hcSetState\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcSetState (&Xhc->Usb2Hc, State);
  return Status;
}
...
{
  DEBUG((DEBUG_INFO, "hcControlTransfer\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcControlTransfer (
    &Xhc->Usb2Hc,
    DeviceAddress,
...
{
  DEBUG((DEBUG_INFO, "hcBulkTransfer\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcBulkTransfer (
    &Xhc->Usb2Hc,
    DeviceAddress,
    EndPointAddress,
    EFI_USB_SPEED_FULL,
...
{
  DEBUG((DEBUG_INFO, "hcAsyncInterruptTransfer\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcAsyncInterruptTransfer (
    &Xhc->Usb2Hc,
    DeviceAddress,
    EndPointAddress,
    IsSlowDevice ? EFI_USB_SPEED_LOW : EFI_USB_SPEED_FULL,
...
{
  DEBUG((DEBUG_INFO, "hcSyncInterruptTransfer\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcSyncInterruptTransfer (
    &Xhc->Usb2Hc,
    DeviceAddress,
    EndPointAddress,
    IsSlowDevice ? EFI_USB_SPEED_LOW : EFI_USB_SPEED_FULL,
...
{
  DEBUG((DEBUG_INFO, "hcIsochronousTransfer\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcIsochronousTransfer (
    &Xhc->Usb2Hc,
    DeviceAddress,
    EndPointAddress,
    EFI_USB_SPEED_FULL,
...
{
  DEBUG((DEBUG_INFO, "hcAsyncIsochronousTransfer\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcAsyncIsochronousTransfer (
    &Xhc->Usb2Hc,
    DeviceAddress,
    EndPointAddress,
    EFI_USB_SPEED_FULL,
...
{
  DEBUG((DEBUG_INFO, "hcGetRootHubPortNumber\n"));
  UINT8 MaxSpeed;
  UINT8 Is64BitCapable;
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcGetCapability (
    &Xhc->Usb2Hc,
    &MaxSpeed,
    PortNumber,
    &Is64BitCapable
...
{
  DEBUG((DEBUG_INFO, "hcGetRootHubPortStatus\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcGetRootHubPortStatus (
    &Xhc->Usb2Hc,
    PortNumber,
    PortStatus
  );
...
{
  DEBUG((DEBUG_INFO, "hcSetRootHubPortFeature\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcSetRootHubPortFeature (
    &Xhc->Usb2Hc,
    PortNumber,
    PortFeature
  );
...
{
  DEBUG((DEBUG_INFO, "hcClearRootHubPortFeature\n"));
  USB_XHCI_INSTANCE *Xhc = XHC_FROM_HC (This);
  EFI_STATUS Status = XhcClearRootHubPortFeature (
    &Xhc->Usb2Hc,
    PortNumber,
    PortFeature
  );
